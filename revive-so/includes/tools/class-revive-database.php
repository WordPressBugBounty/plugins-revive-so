...
class REVIVESO_Database {
			...
			if ( REVIVESO_Admin::is_reviveso_admin_page() ) {
				...
				<div aria-live='assertive' class='reviveso-notice mt-6 pointer-events-none fixed inset-0 flex items-end px-4 py-6 sm:items-start sm:p-6'>
					<div class='flex w-full flex-col items-center space-y-4 sm:items-end'>
						<div class='border-2 border-t-0 border-b-0 border-r-0 border-green-500 pointer-events-auto w-full max-w-sm overflow-hidden rounded-lg bg-white shadow-lg ring-1 ring-black ring-opacity-5'>
							<div class='p-4'>
								<div class='flex items-center'>
									<div class='flex w-0 flex-1 justify-between'>
										<p class='w-0 flex-1 text-sm font-medium text-gray-900'>
											<?php
											esc_html_e( 'Success! Plugin Settings has been imported successfully.', 'revive-so' );
											?>
										</p>
									</div>
								</div>
	...
	public function remove_data() {
		if ( ! isset( $_POST['reviveso_remove_data'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['reviveso_remove_data'] ) ), 'reviveso_remove_data' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
			return;
		}
	...
	public function run_cleanup() {
		if ( ! isset( $_POST['reviveso_remove_meta'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['reviveso_remove_meta'] ) ), 'reviveso_remove_meta' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
			return;
		}
		...
		if ( ! empty( $post_ids ) ) {
			...
			$where = $wpdb->prepare( //phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber
				sprintf( ' WHERE post_id IN ( %s )', $post_ids_placeholders ), //phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
				$post_ids
			);

			$wpdb->query(
				$wpdb->prepare( 
					"DELETE FROM {$wpdb->postmeta} {$where} AND ( meta_key LIKE %s OR meta_key LIKE %s )", //phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
