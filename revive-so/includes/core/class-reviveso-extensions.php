...
class REVIVESO_Extensions {
	...
	public function extensions() {
		...
		$cards = REVIVESO_Tailwind_UI::tailwind_cards();

		?>
		<div class="wrap">
		<div class="reviveso-extensions-header py-3">
			<div class="reviveso-extensions__action_buttons">
				<a href="#" id="reviveso-reload-extensions" class="rounded-full bg-indigo-50 px-2.5 py-1.5 text-sm font-semibold text-indigo-600 shadow-sm hover:bg-indigo-100" data-nonce="
				<?php
				echo wp_create_nonce( 'reviveseo_reload_extensions' )
				?>
				">
					<?php
					echo esc_html__( 'Reload extensions', 'revive-so' );
					?>
				</a>
			</div>
		</div>
		<div class="reviveso-addons-container <?php echo esc_attr( $cards['wrapper'] ); ?>">
			<?php
			// Get the addons.
			...
			if ( ! empty( $addons['all_extensions'] ) ) {
				foreach ( $addons['all_extensions'] as $slug => $addon ) {
					...
					?>
					<div class='reviveso-addon <?php echo esc_attr( $cards['card'] ); ?>'>
						<div class='reviveso-addon-box <?php echo esc_attr( $cards['body'] ); ?>'>
							<div>
								<img src='
								<?php
							...
							<div class='reviveso-addon-content'>
								<h3>
								<?php
									echo esc_html( $extension->get_name() );
								?>
								</h3>
								<span class='reviveso-addon-version'>V 
								<?php
						...
						<div class='reviveso-addon-actions <?php echo esc_attr( $cards['footer'] ); ?>'>
