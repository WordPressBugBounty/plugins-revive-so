...
class REVIVESO_Upsells {
	...
	public function render_upsell_block( $items ) {
		...
		<div class="reviveso-settings-tab-upsell">
			<h3>
			<?php
				esc_html_e( 'Revive.so PRO', 'revive-so' );
			?>
			</h3>
			<p>
			<?php
				esc_html_e( 'Revive.so PRO grants you even more control over your content allowing you to auto post your republished posts to social media and offer even more republishing customizations for your posts.', 'revive-so' );
			?>
			</p>
		</div>
		<?php
	...
	public function render_rewriting_upsell_block( $data ) {
		...
		$class = 'reviveso_rewriting_info reviveso_rewriting_disabled reviveso-form-el';
		?>
		<div class="<?php echo esc_attr( $class ); ?>">
			<p>
				<span class="reviveso_status_dot"></span>
				<?php
				/* translators: 1: opening anchor tag, 2: closing anchor tag */
				printf( esc_html__( 'Rewriting is not active, unlock rewriting with GPT 4 by purchasing %1$s Revive.so Pro %2$s', 'revive-so' ), '<a href="' . esc_url( $link ) . '">', '</a>' );
				?>
			</p>
		</div>
		<?php
	...
	public function upsell_settings( $settings ) {
		...
		$settings['general']['instant_index'] = array(
			'title'       => __( 'Instant Indexer Setup', 'revive-so' ),
			'name'        => __( 'Instant Indexer Setup', 'revive-so' ),
			'fields'      => array(),
			'save_button' => false,
			'type'        => 'upsell',
			'badge'       => __( 'PRO', 'revive-so' ),
		...
		$settings['general']['keyword_pulse'] = array(
			'title'       => __( 'Keyword Pulse Setup', 'revive-so' ),
			'name'        => __( 'Keyword Pulse Setup', 'revive-so' ),
			'fields'      => array(),
			'save_button' => false,
			'type'        => 'upsell',
			'badge'       => __( 'PRO', 'revive-so' ),
